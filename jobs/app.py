...
# g is a global helper that will provide access to the DB throughout the application
import sqlite3
import datetime 
from flask import Flask, render_template, g, request, redirect, url_for

# constant that contains path to the db
PATH = 'db/jobs.sqlite'

# create a Flask app instance
app = Flask(__name__)

# open and close database connection between Flask and Sqlite3
def open_connection():
  connection = getattr(g, '_connection', None)
  if connection == None:
    connection = g._connection = sqlite3.connect(PATH)
  connection.row_factory = sqlite3.Row  # makes accessing data/row indexing easier
  return connection

# query database function
def execute_sql(sql, values=(), commit=False, single=False):
  connection = open_connection()
  cursor = connection.execute(sql, values) # query database function execute
  if commit == True: # query database function results
...
# close the database connection when the app context is no longer relevent/destroyed/tore down
...
# basic route that displays our simplified index.html template
# in the body of the jobs function we return a call to render_template() and pass the parameter 'index.html'
def jobs():
...
def review(employer_id):
  if request.method == 'POST':
    ...
    #  url_for('route', keyword argument)
